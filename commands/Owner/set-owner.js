module.exports = {
  ...
  async execute(client, message, args) {
    ...
    try {
      ...
      tempmsg = await message.channel.send({
        embed: new Discord.MessageEmbed()
          .setTitle("What do you want to do?")
          .setColor(`RANDOM`)
          .setDescription(`1️⃣ **== Add Owner**\n\n📑 **== Show Settings**\n\n**NOTE:**\n> *You can't remove a Owner, which means you need to get in touch with: \`mr.sinre\` to do so!*\n\n\n\n*React with the Right Emoji according to the Right action*`)
          .setFooter(``)
      })
      ...
      if (temptype == "add") {
        ...
        await tempmsg.channel.awaitMessages(m => m.author.id === message.author.id, {
          ...
          .then(collected => {
            ...
            if (user) {
              if (client.config.owners.includes(user.id)) return message.reply({
                embed: new Discord.MessageEmbed()
              ...
              try {
                ...
                fs.writeFile(`./config/bot.js`, ES.stringify(status, null, 3), (e) => {
                  if (e) {
                ...
                return message.reply({
                  embed: new Discord.MessageEmbed()
                    .setTitle("ERROR | Something went wrong, please contact: `CEO│Eul mr.sinre`")
                    .setColor(`RED`)
                    .setDescription(`\`\`\`${String(JSON.stringify(e)).substr(0, 2000)}\`\`\``)
                    .setFooter(``)
                });
        ...
        var embed = new MessageEmbed()
          ...
          .setColor(`RANDOM`)
          .setThumbnail(``)
          .setDescription(`${`<@${client.config.owners.join(">, <@")}>`}`.substr(0, 2048))
          .setFooter(``)
        ...
        return message.reply({
          embed: new Discord.MessageEmbed()
            .setTitle("ERROR | PLEASE CONTACT `CEO│Eul mr.sinre`")
            .setColor(`RED`)
            .setFooter(``)
        });
      }
