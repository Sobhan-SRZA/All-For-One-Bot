 ...
 async execute(client, message, args) {
    ...
    if (!args1) {
      let ticketEmbed = new Discord.MessageEmbed()
        ...
        .setFooter(`Requested by ${message.author.username} | Created By mr.sinre :)`, `${message.author.displayAvatarURL()}`)
        ...
        if (!message.guild.channels.cache.find(x => x.name === ticketName)) {
          ...
          message.channel.send(ticketCreateEmbed, { components: row }).then((msg) => {
            ...
            collect.on('collect', async (x) => {
              x.reply.defer()
              if (x.id === "bale") {
                message.guild.channels.create(`ticket-${message.author.tag}`).then(async (channel) => {
